...
REMOTE_INDEX_URL = (
    "https://github.com/czbiohub/luminoth-uv-imaging/releases/download/v0.4.3/"
    "checkpoints.json"
)
...
def merge_index(local_index, remote_index):
    ...
    remotes_in_local = {
        c["id"]: c for c in local_index["checkpoints"] if c["source"] == "remote"
    }
    ...
    already_downloaded = [
        c
        for c in remotes_in_local.values()
    ...
    if len(missing_ids):
        if len(already_downloaded):
            click.echo(
                "{} remote checkpoints transformed to local.".format(
                    len(already_downloaded)
                )
            )
        click.echo(
            "{} remote checkpoints removed.".format(
...
def get_checkpoint_config(id_or_alias, prompt=True):
    ...
    if prompt and not checkpoint:
        ...
        # try again.
        click.echo("Checkpoint not found. Checking remote repository?")
        db = refresh_remote_index()
        checkpoint = get_checkpoint(db, id_or_alias)
        if not checkpoint:
            # Still not found, abort.
            click.echo("Checkpoint isn't available in remote repository either.")
...
def parse_entries(entries, editing=False):
    ...
    if disallowed:
        click.echo(
            "The following fields may not be set: {}".format(", ".join(disallowed))
        )
        return
...
def list():
    ...
    header = template.format("id", "name", "alias", "source", "status")
...
def info(id_or_alias):
    ...
    checkpoint = get_checkpoint(db, id_or_alias)
    if not checkpoint:
        click.echo("Checkpoint '{}' not found in index.".format(id_or_alias))
        return
    ...
    click.echo("    Number of classes: {}".format(checkpoint["dataset"]["num_classes"]))
    ...
    if checkpoint["source"] == "remote":
        click.echo("Source: {} ({})".format(checkpoint["source"], checkpoint["status"]))
...
def create(config_files, override_params, entries):
    ...
    metadata = {
        ...
        "dataset": {
            ...
            "num_classes": (num_classes or entries.get("dataset.num_classes", None)),
        },
...
def delete(id_or_alias):
    ...
    if checkpoint["source"] == "local":
        db["checkpoints"] = [
            cp for cp in db["checkpoints"] if not cp["id"] == checkpoint["id"]
        ]
    else:  # Remote.
    ...
    try:
        ...
        click.echo("Skipping files deletion; not present in {}.".format(path))
...
def export(id_or_alias, output):
    ...
    tar_path = os.path.join(
        os.path.abspath(os.path.expanduser(output)), "{}.tar".format(checkpoint["id"])
    )
...
def import_(path):
    ...
    try:
        with tarfile.open(path) as f:
            ...
            metadata = json.loads(f.extractfile("metadata.json").read().decode("utf-8"))
    except tarfile.ReadError:
        click.echo("Invalid file. Is it an exported checkpoint?")
        return
    except KeyError:
        click.echo(
            "Tar file doesn't contain `metadata.json`. " "Is it an exported checkpoint?"
        )
        return
    ...
    if checkpoint:
        click.echo("Checkpoint '{}' already found in index.".format(metadata["id"]))
        return
