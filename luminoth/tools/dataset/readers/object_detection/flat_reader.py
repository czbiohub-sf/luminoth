...
class FlatReader(ObjectDetectionReader):
    ...
    def _get_annotation_path(self, annotation_id):
        return os.path.join(
            self._get_split_path(), "{}.{}".format(annotation_id, self._annotation_type)
        )
    ...
    def _get_image_path(self, image_id):
        ...
        possible_files = [f for f in self.files if f.startswith("{}.".format(image_id))]
        if len(possible_files) == 0:
            return
        elif len(possible_files) > 1:
            tf.logging.warning(
                "Image {} matches with {} files.".format(image_id, len(possible_files))
            )
    ...
    def _read_annotation(self, annotation_path):
        if self._annotation_type == "json":
            ...
            raise ValueError(
                "Annotation type {} not supported".format(self._annotation_type)
            )
