...
REQUIRED_KEYS = set(["width", "height", "depth", "filename", "image_raw", "gt_boxes"])
...
class ObjectDetectionWriter(BaseWriter):
    ...
    def __init__(self, reader, output_dir, split="data"):
        ...
        if not isinstance(reader, ObjectDetectionReader):
            raise ValueError("Saver needs a valid ObjectDetectionReader subclass")
    ...
    def save(self):
        """"""
        tf.logging.info(
            'Saving split "{}" in output_dir = {}'.format(self._split, self._output_dir)
        )
        ...
        json.dump(
            [self._reader.pretty_name(label) for label in self._reader.classes],
            tf.gfile.GFile(classes_file, "w"),
        )

        record_file = os.path.join(self._output_dir, "{}.tfrecords".format(self._split))
        writer = tf.python_io.TFRecordWriter(record_file)

        tf.logging.debug("Found {} images.".format(self._reader.total))

        with click.progressbar(
            self._reader.iterate(), length=self._reader.total
        ) as record_list:
            ...
            if self._output_dir.startswith("gs://"):
                tf.logging.info(
                    "Saving tfrecord to Google Cloud Storage. " "It may take a while."
                )
            writer.close()
        ...
        tf.logging.info(
            'Saved {} records to "{}"'.format(self._reader.yielded_records, record_file)
        )
    ...
    def _validate_record(self, record):
        ...
        if record_keys != REQUIRED_KEYS:
            raise InvalidRecord("Missing keys: {}".format(REQUIRED_KEYS - record_keys))
        ...
        for gt_box in record["gt_boxes"]:
            ...
            if gt_keys != REQUIRED_GT_KEYS:
                raise InvalidRecord(
                    "Missing gt boxes keys {}".format(REQUIRED_GT_KEYS - gt_keys)
                )
