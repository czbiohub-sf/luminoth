...
SCALE_TIERS = ["BASIC", "STANDARD_1", "PREMIUM_1", "BASIC_GPU", "BASIC_TPU", "CUSTOM"]
MACHINE_TYPES = [
    "standard",
    "large_model",
    "complex_model_s",
...
def build_package(bucket, base_path):
    ...
    if os.path.isfile(setup_file):
        ...
        click.echo(
            'Generating "{}" for installing luminoth=={}.'.format(
...
class ServiceAccount(object):
    ...
    def __init__(self):
        try:
            ...
            self.credentials = (
                service_account.ServiceAccountCredentials.from_json_keyfile_dict(data)
            )
    ...
    def validate_region(self, region):
        regionrequest = (
            self.cloud_service("compute")
            .regions()
            .get(region=region, project=self.project_id)
        )
        try:
...
def upload_file(bucket, base_path, file_path):
    ...
    click.echo(
        'Uploading file: "{}"\n\t-> to "gs://{}/{}"'.format(filename, bucket.name, path)
    )
...
@click.option(
    "--job-id",
    help="Identifies the training job in Google Cloud. Will use it to name the folder where checkpoints and logs will be stored, except when resuming a previous training job.",
)  # noqa
@click.option(
    "--resume",
    "resume_job_id",
    help="Id of the previous job to resume (start from last stored checkpoint). In case you are resuming multiple times, must always point to the first job (ie. the one that first created the checkpoint).",
)  # noqa
@click.option(
    "--bucket",
    "bucket_name",
    help="Bucket where to create the folder to save checkpoints and logs. If resuming a job, it must match the bucket used for the original job.",
)  # noqa
@click.option(
    "--region", default="us-central1", help="Region in which to run the job."
)  # noqa
@click.option(
    "--dataset",
    help="Complete path (bucket included) to the folder where the dataset is located (TFRecord files).",
)  # noqa
@click.option(
    "config_files",
    "--config",
    ...
    required=True,
    multiple=True,
    help="Path to config to use in training.",
)  # noqa
@click.option(
    "--scale-tier", default=DEFAULT_SCALE_TIER, type=click.Choice(SCALE_TIERS)
)  # noqa
@click.option(
    "--master-type", default=DEFAULT_MASTER_TYPE, type=click.Choice(MACHINE_TYPES)
)  # noqa
@click.option(
    "--worker-type", default=DEFAULT_WORKER_TYPE, type=click.Choice(MACHINE_TYPES)
)  # noqa
@click.option("--worker-count", default=DEFAULT_WORKER_COUNT, type=int)
@click.option(
    "--parameter-server-type", default=DEFAULT_PS_TYPE, type=click.Choice(MACHINE_TYPES)
)  # noqa
...
def train(
    ...
    jobrequest = (
        cloudml.projects()
        .jobs()
        .create(body=job_spec, parent="projects/{}".format(account.project_id))
    )
...
@click.option(
    "--job-id",
    help="Job Id for naming the folder where the results of the evaluation will be stored.",
)  # noqa
@click.option(
    "--train-folder",
    "train_folder",
    required=True,
    help="Complete path (bucket included) where the training results are stored (config.yml should live here).",
)  # noqa
@click.option(
    "--bucket",
    "bucket_name",
    help="The bucket where the evaluation results were stored.",
)  # noqa
@click.option(
    "dataset_split", "--split", default="val", help="Dataset split to use."
)  # noqa
@click.option(
    "--region", default="us-central1", help="Region in which to run the job."
)  # noqa
@click.option(
    "--machine-type", default=DEFAULT_MASTER_TYPE, type=click.Choice(MACHINE_TYPES)
)  # noqa
@click.option(
    "--rebuild",
    default=False,
    is_flag=True,
    help="Rebuild Luminoth package for evaluation. If not, will use the same package that was used for training.",
)  # noqa
@check_dependencies
def evaluate(
    job_id, train_folder, bucket_name, dataset_split, region, machine_type, rebuild
):
    account = ServiceAccount()
    account.validate_region(region)
    ...
    if bucket_name is None:
        bucket_name = "luminoth-{}".format(account.client_id)
        click.echo(
            'Bucket name not specified. Using "{}".'.format(bucket_name), err=True
        )
    ...
    if rebuild:
        ...
        train_packages_dir = "{}/{}".format(train_folder, DEFAULT_PACKAGES_PATH)

        try:
            package_files = tf.gfile.ListDirectory(train_packages_dir)
            package_filename = [n for n in package_files if n.endswith("tar.gz")][0]
            full_package_path = "{}/{}".format(train_packages_dir, package_filename)
...
@click.option(
    "--running", is_flag=True, help="List only jobs that are running."
)  # noqa
@check_dependencies
def jobs(running):
    account = ServiceAccount()
    cloudml = account.cloud_service("ml")
    request = (
        cloudml.projects().jobs().list(parent="projects/{}".format(account.project_id))
    )
...
@click.option(
    "--polling-interval", default=30, help="Polling interval in seconds."
)  # noqa
@check_dependencies
