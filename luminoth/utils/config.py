...
def load_config_files(filename_or_filenames, warn_overwrite=True):
    if isinstance(filename_or_filenames, list) or isinstance(
        filename_or_filenames, tuple
    ):
    ...
    for filename in filenames:
        ...
        config = merge_into(
            new_config, config, overwrite=True, warn_overwrite=warn_overwrite
        )
    return config
