...
def get_diff():
    try:
        return (
            subprocess.check_output(["git", "diff"], cwd=CURRENT_DIR)
            .strip()
            .decode("utf-8")
        )
    except:  # noqa
...
def get_luminoth_version():
    try:
        return (
            subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=CURRENT_DIR)
            .strip()
            .decode("utf-8")
        )
    except:  # noqa
...
def save_run(
    config, environment=None, comment=None, extra_config=None, filename=DEFAULT_FILENAME
):
