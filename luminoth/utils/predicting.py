...
class PredictorNetwork(object):
    ...
    def __init__(self, config):
        ...
        with graph.as_default():
            ...
            image_tf, _, process_meta = dataset.preprocess(self.image_placeholder)
    ...
    def predict_image(self, image):
        fetched = self.session.run(
            self.fetches, feed_dict={self.image_placeholder: np.array(image)}
        )
        ...
        objects = [[int(round(coord)) for coord in obj] for obj in objects.tolist()]
