...
class ImageTest(tf.test.TestCase):
    ...
    def _resize_image(
        self, image_array, boxes_array=None, min_size=None, max_size=None
    ):
    ...
    def _rot90(self, image_array, boxes_array):
        with self.test_session() as sess:
            rotate = rot90(image_array, bboxes=boxes_array)
    ...
    def _random_patch_gaussian(self, image_array, boxes_array):
        with self.test_session() as sess:
            gaussian = random_patch_gaussian(image_array, bboxes=boxes_array)
    ...
    def _random_distort(self, image, config, bboxes=None):
        with self.test_session() as sess:
            distort = random_distortion(image, bboxes=bboxes, seed=0, **config)
    ...
    def testResizeOnlyImage(self):
        ...
        resized_image, _, scale = self._resize_image(self._gen_image(100, 1024, 3))
        self.assertAllEqual(resized_image.shape, (100, 1024, 3))
        ...
        self.assertAllEqual(resized_image.shape, (97, 1000, 3))
        ...
        self.assertAllEqual(resized_image.shape, (120, 1228, 3))
        ...
        self.assertAllEqual(resized_image.shape, (50, 512, 3))
        ...
        self.assertAllEqual(resized_image.shape, (200, 2048, 3))
        self.assertAlmostEqual(scale, 2.0)

        # min and max invariant changes, both changes negate themselves.
        change = 1.1
        resized_image, _, scale = self._resize_image(
            self._gen_image(100, 200, 3),
            min_size=int(100 * change),
            max_size=round(200 / change),
        )
        self.assertAllEqual(resized_image.shape, (100, 200, 3))
        ...
        self.assertAllEqual(resized_image.shape, (100, 200, 3))  # No change
        self.assertAlmostEqual(int(scale), 1)

        resized_image, _, scale = self._resize_image(
            self._gen_image(100, 200, 3), min_size=600, max_size=1000
        )
        self.assertAllEqual(
            resized_image.shape, (100 * 6, 200 * 6, 3)  # We end up bigger than maximum.
        )
        self.assertAlmostEqual(scale, 6.0)

        resized_image, _, scale = self._resize_image(
            self._gen_image(2000, 600, 3), min_size=600, max_size=1000
        )
        self.assertAllEqual(
            resized_image.shape, (1000, 300, 3)  # We end up smaller than minimum image.
        )
        self.assertAlmostEqual(scale, 0.5)
    ...
    def testResizeImageBoxes(self):
        ...
        _, resized_boxes, scale = self._resize_image(image, boxes, max_size=50)
        ...
        _, resized_boxes, scale = self._resize_image(image, boxes, max_size=25)
    ...
    def testPatchImageUpdateCondition(self):
        ...
        with self.test_session() as sess:
            ...
            feed_dict = {image_ph: image, bboxes_ph: bboxes}
    ...
    def testFlipImageBoxes(self):
        ...
        image = self._gen_image(100, 100, 3)
        boxes = np.array([[0, 0, 99, 99, -1]])
        flipped_image, flipped_boxes = self._flip_image(image, boxes, left_right=True)
        ...
        image = self._gen_image(100, 100, 3)
        boxes = np.array([[0, 0, 99, 99, -1]])
        flipped_image, flipped_boxes = self._flip_image(image, boxes, up_down=True)
        self.assertAllClose(boxes, flipped_boxes)
        # Check that sum of columns is not modified, just the order.
        ...
        # Check that sum of columns is not modified, just the order.
        self.assertAllClose(np.array([[89.0, 89.0, 99.0, 99.0, -1.0]]), flipped_boxes)

    def testFlipBboxesDiffDtype(self):
        image = self._gen_image(100, 100, 3)
        boxes = np.array([[25, 14, 63, 41, -1]])
        _, flipped_boxes_float = self._flip_image(
            image, boxes, up_down=True, left_right=True, bboxes_dtype=tf.float32
        )

        _, flipped_boxes_int = self._flip_image(
            image, boxes, up_down=True, left_right=True, bboxes_dtype=tf.int32
        )

        # Check that using different types is exactly the same.
        self.assertAllClose(flipped_boxes_float, flipped_boxes_int)
    ...
    def testRandomPatchImageBboxes(self):
        ...
        bboxes_w_label = tf.concat(
            [bboxes, tf.fill((bboxes.shape[0], 1), label)], axis=1
        )
        config = self._random_patch_config
        ret_image, ret_bboxes = self._random_patch(image, config, bboxes_w_label)
        # Assertions
        ...
        self.assertTrue(np.all(ret_bboxes[:, 0] <= ret_image.shape[1]))
        self.assertTrue(np.all(ret_bboxes[:, 1] <= ret_image.shape[0]))
        self.assertTrue(np.all(ret_bboxes[:, 2] <= ret_image.shape[1]))
        self.assertTrue(np.all(ret_bboxes[:, 3] <= ret_image.shape[0]))
    ...
    def testRandomPatchLargerThanImage(self):
        ...
        im_shape = (600, 800, 3)
        total_boxes = 5
        config = EasyDict({"min_height": 900, "min_width": 900})
        label = 3
        image, bboxes = self._get_image_with_boxes(im_shape, total_boxes)
        # Add a label to each bbox.
        bboxes_w_label = tf.concat(
    ...
    def testRandomResizeImageBboxes(self):
        ...
        ret_image, ret_bboxes = self._random_resize(image, config, bboxes_w_label)
        # Assertions
        self.assertEqual(ret_bboxes.shape[0], total_boxes)
        self.assertTrue(np.all(np.asarray(ret_image.shape[:2]) >= config.min_size))
        self.assertTrue(np.all(np.asarray(ret_image.shape[:2]) <= config.max_size))
    ...
    def testRandomDistort(self):
        ...
        ret_image, ret_bboxes = self._random_distort(image, config, bboxes_w_label)
        # Assertions
        self.assertEqual(im_shape, ret_image.shape)
        self.assertAllEqual(bboxes, ret_bboxes[:, :4])
        ...
        config = EasyDict(
            {
                ...
                "contrast": {"lower": 0.99999, "upper": 1.00001},
            }
        )
        label = 3
        image, bboxes = self._get_image_with_boxes(im_shape, total_boxes)
    ...
    def testRotate90(self):
        ...
        label = 3
        width = image.shape[1]
        bboxes_w_label = tf.concat([boxes, tf.fill((boxes.shape[0], 1), label)], axis=1)
        ...
        for i in range(total_boxes):
            ...
            self.assertEqual(int(round(boxes[i, 0] + ret_bboxes[i, 3])), width)
    ...
    def testRandomPatchGaussian(self):
        ...
        self.assertAllEqual(boxes, ret_bboxes[:, :4])
        for i in range(total_boxes):
            self.assertEqual(ret_bboxes[i, 4], label)
