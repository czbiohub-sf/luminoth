...
class RPNProposalTest(tf.test.TestCase):
    ...
    def _run_rpn_proposal(
        self, all_anchors, rpn_cls_prob, config, gt_boxes=None, rpn_bbox_pred=None
    ):
        """
        ...
        if rpn_bbox_pred is None and gt_boxes is not None:
            ...
            rpn_bbox_pred_tf = tf.placeholder(tf.float32, shape=rpn_bbox_pred.shape)
    ...
    def testNMSThreshold(self):
        ...
        rpn_cls_prob = np.array([[0.8, 0.2], [0.1, 0.9], [0.4, 0.6], [0.2, 0.8]])
        config = self.config
        ...
        self.assertEqual(results["proposals"].shape, (2, 4))

        self.assertAllClose(results["scores"], [0.9, 0.8])
        ...
        # Also check that we get the corrects scores.
        self.assertEqual(results["proposals"].shape, (3, 4))

        self.assertAllClose(results["scores"], [0.9, 0.8, 0.2])
        ...
        self.assertEqual(results["proposals"].shape, (4, 4))
    ...
    def testOutsidersAndTopN(self):
        ...
        rpn_cls_prob = np.array([[0.3, 0.7], [0.4, 0.6], [0.9, 0.1], [0.8, 0.2]])
        ...
        self.assertEqual(results["unsorted_proposals"].shape, (4, 4))

        # Also check that we get the corrects scores.
        self.assertAllClose(results["scores"], [0.7, 0.6, 0.2])
        ...
        self.assertAllEqual(results["proposals"].shape, (2, 4))

        self.assertEqual(results["unsorted_proposals"].shape, (4, 4))

        # Also check that we get the corrects scores.
        self.assertAllClose(results["scores"], [0.7, 0.6])

        # Sorted
        self.assertAllClose(results["sorted_top_scores"], [0.7, 0.6, 0.2, 0.1])
        ...
        # Filter pre nms
        self.assertEqual(results["sorted_top_proposals"].shape, (2, 4))

        # Also check that we get the corrects scores.
        self.assertAllClose(results["scores"], [0.7, 0.6])

        self.assertAllClose(results["sorted_top_scores"], [0.7, 0.6])
        ...
        self.assertAllEqual(results["proposals"].shape, (1, 4))

        self.assertEqual(results["sorted_top_proposals"].shape, (2, 4))

        # Also check that we get the corrects scores.
        self.assertAllClose(results["scores"], [0.7])
    ...
    def testNegativeArea(self):
        ...
        self.assertEqual(results["unsorted_proposals"].shape, (2, 4))
    ...
    def testNegativeAreaProposals(self):
        ...
        results = self._run_rpn_proposal(
            all_anchors, rpn_cls_prob, self.config, rpn_bbox_pred=rpn_bbox_pred
        )

        self.assertEqual(results["unsorted_proposals"].shape, (3, 4))
    ...
    def testClippingOfProposals(self):
        ...
        rpn_bbox_pred = np.array(
            [  # This is set to zeros so when decode is
        ...
        self.assertAllEqual(results_before["unsorted_proposals"], clipped_proposals)
        ...
        self.assertTrue((proposals < np.array(self.im_size + self.im_size)).all())
        ...
        self.assertAllEqual(
            results_after["unsorted_proposals"], results_after["proposals_unclipped"]
        )
    ...
    def testFilterOutsideAnchors(self):
        ...
        self.assertAllEqual(
            results_without_filter["all_proposals"].shape, all_anchors.shape
        )
        ...
        self.assertAllEqual(results_with_filter["all_proposals"].shape, (2, 4))
