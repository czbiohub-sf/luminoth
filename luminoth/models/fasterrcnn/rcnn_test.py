...
class RCNNTest(tf.test.TestCase):
    def setUp(self):
        ...
        self._proposals_ph = tf.placeholder(tf.float32, shape=self._proposals_shape)

        self._image_shape_shape = (2,)
        self._image_shape_ph = tf.placeholder(tf.float32, shape=self._image_shape_shape)

        self._gt_boxes_shape = (self._total_num_gt, 5)
        self._gt_boxes_ph = tf.placeholder(tf.float32, shape=self._gt_boxes_shape)
    ...
    def _check_returning_shapes(self, prediction_dict, training=False):
        ...
        self.assertEqual(cls_prob_shape, (num_proposals, self._num_classes + 1))

        self.assertEqual(bbox_offsets_shape, (num_proposals, self._num_classes * 4))

        if training:
            cls_target_shape = prediction_dict["target"]["cls"].shape
            self.assertEqual(cls_target_shape, (num_proposals,))

            bbox_offsets_trgt_shape = prediction_dict["target"]["bbox_offsets"].shape
    ...
    def testNumberOfObjects(self):
        ...
        # Assertions
        self.assertLessEqual(
            prediction_dict["objects"].shape[0], self._num_proposals * self._num_classes
        )
    ...
    def testLoss(self):
        ...
        bbox_offsets_target_ph = tf.placeholder(tf.float32, bbox_offsets_target_shape)
        ...
        bbox_offsets_target = np.zeros(bbox_offsets_target_shape, dtype=np.float32)
        for i in range(self._num_proposals):
            ...
            for j in range(4):
                this_coord = np.random.randint(low=0, high=np.amax(self._image_shape))
        ...
        self.assertAlmostEqual(
            loss_dict["rcnn_cls_loss"], 0, delta=self._equality_delta
        )
        self.assertAlmostEqual(
            loss_dict["rcnn_reg_loss"], 0, delta=self._equality_delta
        )
