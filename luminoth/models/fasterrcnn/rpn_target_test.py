...
class RPNTargetTest(tf.test.TestCase):
    ...
    def _run_rpn_target(self, anchors, gt_boxes, config):
        ...
        labels, bbox_targets, max_overlaps = model(all_anchors, gt_boxes_tf, im_size)
    ...
    def testBorderOutsiders(self):
        ...
        # the image.
        self.assertAllEqual(labels, np.array([1, 0, 0, -1, 1]))
        ...
        # Assert bbox targets we are ignoring are zero.
        self.assertAllEqual(bbox_targets[1:4], np.zeros((3, 4)))
        ...
        # Assert bbox targets we are ignoring are zero.
        self.assertAllEqual(bbox_targets[1:], np.zeros((4, 4)))
    ...
    def testWithNoClearMatch(self):
        ...
        self.assertAllEqual(labels, np.array([1, 0]))
        ...
        # clobbering enabled.
        self.assertAllEqual(labels, np.array([0, 0]))
    ...
    def testWithMultipleGTBoxes(self):
        ...
        self.assertAllEqual(labels, np.array([0, 1, -1]))

        # Assert bbox targets we are ignoring are zero.
        self.assertAllEqual(bbox_targets[0], [0, 0, 0, 0])

        self.assertAllEqual(bbox_targets[2], [0, 0, 0, 0])
