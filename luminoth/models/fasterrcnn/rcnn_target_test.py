...
class RCNNTargetTest(tf.test.TestCase):
    def setUp(self):
        ...
        self._shared_model = RCNNTarget(self._num_classes, self._config, seed=0)
    ...
    def testBasic(self):
        ...
        gt_boxes = tf.constant(
            [(20, 20, 80, 100, self._placeholder_label)], dtype=tf.float32
        )
        ...
        self.assertAllClose(
            proposals_label,
            np.array([0.0, self._placeholder_label + 1, -1.0]),
            atol=self._equality_delta,
        )

        self.assertEqual(
            proposals_label[proposals_label >= 0].shape[0], self._config.minibatch_size
        )
    ...
    def testEmptyCase(self):
        ...
        gt_boxes = tf.constant(
            [(423, 30, 501, 80, self._placeholder_label)], dtype=tf.float32
        )
        ...
        self.assertAlmostEqual(
            proposals_label[2], self._placeholder_label + 1, delta=self._equality_delta
        )
    ...
    def testAbsolutelyEmptyCase(self):
        ...
        gt_boxes = tf.constant(
            [(40, 90, 100, 105, self._placeholder_label)], dtype=tf.float32
        )
        ...
        self.assertLessEqual(
            foreground_number, np.floor(foreground_fraction * minibatch_size)
        )
        self.assertLessEqual(background_number, minibatch_size - foreground_number)
    ...
    def testMultipleOverlap(self):
        ...
        gt_boxes = tf.constant(
            [(200, 300, 250, 390, self._placeholder_label)], dtype=tf.float32
        )
    ...
    def testOddMinibatchSize(self):
        ...
        self.assertEqual(
            proposals_label[proposals_label >= 0].shape[0], config.minibatch_size
        )
    ...
    def testBboxTargetConsistency(self):
        ...
        self.assertAllEqual(foreground_idxs, non_empty_bbox_target_idxs)
    ...
    def testMultipleGtBoxes(self):
        ...
        self.assertAllClose(
            ...
            np.add([2.0, 1.0, 1.0, 1.0, 2.0, 0.0, 1.0, 0.0], 1),
    ...
    def testNonZeroForegrounds(self):
        ...
        for _ in range(number_of_iterations):
            im_shape = np.random.randint(low=600, high=980, size=2, dtype=np.int32)
            ...
            gt_boxes_w_label = np.concatenate(
                [gt_boxes, [[self._placeholder_label]] * total_boxes], axis=1
            ).astype(np.float32)
            ...
            (proposals_label, _) = self._run_rcnn_target(
                self._shared_model, gt_boxes_w_label, proposed_boxes.astype(np.float32)
            )
            # Assertion
    ...
    def testCorrectBatchSize(self):
        ...
        gt_boxes = tf.constant([[10, 10, 20, 20, 0]], dtype=tf.float32)

        proposed_boxes_backgrounds = [[21, 21, 30, 30]] * 100
        proposed_boxes_foreground = [[11, 11, 19, 19]] * 100

        proposed_boxes = tf.constant(
            proposed_boxes_backgrounds + proposed_boxes_foreground, dtype=tf.float32
        )
    ...
    def testLabelPriority(self):
        ...
        gt_boxes = tf.constant(
            [[10, 10, 20, 20, first_label], [10, 10, 30, 30, second_label]],
            dtype=tf.float32,
        )
        ...
        (proposals_label, _) = self._run_rcnn_target(model, gt_boxes, proposed_boxes)

        num_first_label = len(proposals_label[proposals_label == first_label + 1])
        num_second_label = len(proposals_label[proposals_label == second_label + 1])
