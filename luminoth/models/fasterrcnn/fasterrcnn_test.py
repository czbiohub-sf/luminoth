...
class FasterRCNNNetworkTest(tf.test.TestCase):
        ...
        self.config = EasyDict(
            {
                ...
                "dataset": {
                    ...
                    "data_augmentation": {"flip": {"prob": 1}},
                },
                "model": {
                    "network": {"num_classes": 20, "with_rcnn": True},
                    "anchors": {
                        "base_size": 256,
                        "scales": [0.5, 1, 2],
                        "ratios": [0.5, 1, 2],
    ...
    def testBasic(self):
        ...
        self.assertEqual(class_prediction["objects"].shape[1], 4)
        ...
        self.assertEqual(
            class_prediction["labels"].shape[0], class_prediction["objects"].shape[0]
        )

        # Check that every object label is less or equal than 'num_classes'
        self.assertTrue(
            np.less_equal(
                class_prediction["labels"], self.config.model.network.num_classes
            ).all()
        )
        ...
        self.assertEqual(rpn_prediction["proposals"].shape[1], 4)
    ...
    def testAnchors(self):
        ...
        self.assertAllEqual(
            clip_boxes(results["all_anchors"], self.image_size), results["all_anchors"]
        )
        ...
        self.assertAllEqual(np.unique(anchor_widths), np.unique(anchor_heights))
    ...
    def set_prediction_dicts(self):
        ...
        prediction_dict_random = {
            ...
            "classification_prediction": {
                "rcnn": {"cls_score": None, "bbox_offsets": None},
                "target": {},
                "_debug": {"losses": {}},
            },
        }
        ...
        rpn_cls_score = tf.cast(
            tf.one_hot(
                tf.cast(tf.mod(tf.identity(rpn_cls_target) + 1, 2), tf.int32),
                depth=2,
                on_value=10,
            ),
            tf.float32,
        ...
        rpn_cls_perf_score = tf.cast(
            tf.one_hot(
                tf.cast(tf.identity(rpn_cls_target), tf.int32), depth=2, on_value=100
            ),
            tf.float32,
        )
        ...
        prediction_dict_random["rpn_prediction"]["rpn_cls_score"] = rpn_cls_score
        prediction_dict_random["rpn_prediction"]["rpn_cls_target"] = rpn_cls_target
        prediction_dict_random["rpn_prediction"]["rpn_bbox_target"] = rpn_bbox_target
        prediction_dict_random["rpn_prediction"]["rpn_bbox_pred"] = rpn_bbox_predictions

        prediction_dict_perf["rpn_prediction"]["rpn_cls_score"] = rpn_cls_perf_score
        prediction_dict_perf["rpn_prediction"]["rpn_cls_target"] = rpn_cls_target
        prediction_dict_perf["rpn_prediction"]["rpn_bbox_target"] = rpn_bbox_target
        prediction_dict_perf["rpn_prediction"]["rpn_bbox_pred"] = rpn_bbox_target
        ...
        prediction_dict_perf["classification_prediction"][
            "target"
        ] = prediction_dict_random["classification_prediction"]["target"].copy()
        ...
        # as the target one
        target_bbox_offsets = prediction_dict_random["classification_prediction"][
            "target"
        ]["bbox_offsets"]
    ...
    def testCECELoss(self):
        ...
        loss_perfect = self._get_losses(config, prediction_dict_perf, image_size)
        loss_random = self._get_losses(config, prediction_dict_random, image_size)
        ...
        for loss in loss_random:
            assert loss_random_compare[loss] - tol < loss_random[loss]
            assert loss_random_compare[loss] + tol > loss_random[loss]
    ...
    def testFocalFocalLoss(self):
        ...
        for loss in loss_random:
            assert loss_random_compare[loss] - tol < loss_random[loss]
            assert loss_random_compare[loss] + tol > loss_random[loss]
            self.assertEqual(float(loss_perfect[loss]), 0, loss)
    ...
    def _assert_sequential_values(self, values, delta=1):
        ...
        paired_values = np.column_stack((unique_values[:-1], unique_values[1:]))
        self.assertAllEqual(
