...
class RPNTest(tf.test.TestCase):
        ...
        self.config = EasyDict(
            {
                ...
                "l1_sigma": 3.0,
                "loss": {"type": "cross_entropy", "weight": [0.5, 1]},
    ...
    def testBasic(self):
        ...
        layers = model(pretrained_output, image_shape, all_anchors, gt_boxes=gt_boxes)
        ...
        with self.test_session() as sess:
            ...
            layers_inst = sess.run(
                ...
                feed_dict={
                    ...
                    gt_boxes: generate_gt_boxes(gt_boxes_shape[0], image_shape_val),
        ...
        self.assertGreaterEqual(self.config.proposals.post_nms_top_n, total_proposals)
        ...
        self.assertEqual(layers_inst["rpn_bbox_pred"].shape, (total_anchors, 4))

        # Check that we get a target for each regression for each anchor.
        self.assertEqual(layers_inst["rpn_bbox_target"].shape, (total_anchors, 4))

        # Check that we get a target class for each anchor.
        self.assertEqual(layers_inst["rpn_cls_target"].shape, (total_anchors,))

        # Check that targets are composed of [-1, 0, 1] only.
        rpn_cls_target = layers_inst["rpn_cls_target"]
        self.assertEqual(tuple(np.sort(np.unique(rpn_cls_target))), (-1, 0.0, 1.0))

        batch_cls_target = rpn_cls_target[
            (rpn_cls_target == 0.0) | (rpn_cls_target == 1.0)
        ]

        # Check that the non negative target class are exactly the size
        # as the minibatch
        self.assertEqual(batch_cls_target.shape, (self.config.target.minibatch_size,))

        # Check that we get upto foreground_fraction of positive anchors.
        self.assertLessEqual(
            batch_cls_target[batch_cls_target == 1.0].shape[0]
