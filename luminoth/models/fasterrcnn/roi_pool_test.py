...
class ROIPoolingTest(tf.test.TestCase):
    ...
    def _run_roi_pooling(self, roi_proposals, pretrained, config):
        roi_proposals_tf = tf.placeholder(tf.float32, shape=roi_proposals.shape)
    ...
    def testBasic(self):
        ...
        results = self._run_roi_pooling(roi_proposals, self.pretrained, self.config)
        ...
        self.assertEqual(results["crops"].shape, (4, 4, 4, 1))

        # Check that roi_pool has the correct shape. This is (4, 2, 2, 1)
        # because we have 4 proposals, 'pool size' = 2x2.
        self.assertEqual(results["roi_pool"].shape, (4, 2, 2, 1))

        results["roi_pool"] = np.squeeze(results["roi_pool"], axis=3)
        # Check that max polling returns only 'multiplier_a'
        self.assertAllEqual(results["roi_pool"][0], np.ones((2, 2)) * self.multiplier_a)

        # Check that max polling returns only 'multiplier_b'
        self.assertAllEqual(results["roi_pool"][1], np.ones((2, 2)) * self.multiplier_b)

        # Check that max polling returns only 'multiplier_c'
        self.assertAllEqual(results["roi_pool"][2], np.ones((2, 2)) * self.multiplier_c)

        # Check that max polling returns only 'multiplier_d'
        self.assertAllEqual(results["roi_pool"][3], np.ones((2, 2)) * self.multiplier_d)
    ...
    def testMaxPoolingWithoutInterpolation(self):
        ...
        # a column of 'two'.
        self.assertAllEqual(results["roi_pool"][0], np.array([[a, b], [a, b]]))

        # Check that max polling returns a row of 'one' and
        # a row of 'three'.
        self.assertAllEqual(results["roi_pool"][1], np.array([[a, a], [c, c]]))

        # Check that max polling returns a row of 'one' and
        # a row of 'three'.
        self.assertAllEqual(results["roi_pool"][2], np.array([[b, b], [d, d]]))

        # Check that max polling returns a column of 'three' and
        # a column of 'four'.
        self.assertAllEqual(results["roi_pool"][3], np.array([[c, d], [c, d]]))
    ...
    def testMaxPoolingWithInterpolation(self):
        ...
        self.assertTrue(np.greater_equal(results["roi_pool"][0], a).all())

        self.assertTrue(np.less_equal(results["crops"][0], b).all())

        # Check that max polling returns values greater or equal
        # than 'a' and crops returns values lower or equal than 'c'
        self.assertTrue(np.greater_equal(results["roi_pool"][1], a).all())

        self.assertTrue(np.less_equal(results["crops"][1], c).all())

        # Check that max polling returns values greater or equal
        # than 'b' and crops returns values lower or equal than 'd'
        self.assertTrue(np.greater_equal(results["roi_pool"][2], b).all())

        self.assertTrue(np.less_equal(results["crops"][2], d).all())

        # Check that max polling returns values greater or equal
        # than 'c' and crops returns values lower or equal than 'd'
        self.assertTrue(np.greater_equal(results["roi_pool"][3], c).all())

        self.assertTrue(np.less_equal(results["crops"][3], d).all())
