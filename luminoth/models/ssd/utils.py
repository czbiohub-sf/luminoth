...
def generate_anchors_reference(ratios, scales, num_anchors, feature_map_shape):
    ...
    if len(scales) > 1:
        widths[0] = heights[0] = np.sqrt(scales[0] * scales[1]) * feature_map_shape[0]
    # The last endpoint
    else:
...
def generate_raw_anchors(
    feature_maps, anchor_min_scale, anchor_max_scale, anchor_ratios, anchors_per_point
):
    """
...
def generate_anchors_per_feat_map(feature_map_shape, anchor_reference):
    ...
    with tf.variable_scope("generate_anchors"):
        ...
        all_anchors = np.expand_dims(anchor_reference, axis=0) + np.expand_dims(
            shifts, axis=1
        )
        # Flatten
