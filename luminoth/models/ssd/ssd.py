...
from luminoth.models.ssd.utils import generate_raw_anchors, adjust_bboxes
from luminoth.utils.losses import smooth_l1_loss, CROSS_ENTROPY, FOCAL, focal_loss
...
class SSD(snt.AbstractModule):
    ...
    def __init__(self, config, name="ssd"):
        ...
        tf.logging.info("Classification loss type in SSD is {}".format(self.loss_type))
    ...
    def _build(self, image, gt_boxes=None, is_training=False):
        ...
        if gt_boxes is not None:
            ...
            with tf.name_scope("hard_negative_mining_filter"):
                ...
                class_targets = tf.boolean_mask(class_targets, predictions_filter)
                class_scores = tf.boolean_mask(class_scores, predictions_filter)
                class_probabilities = tf.boolean_mask(
                    class_probabilities, predictions_filter
                )
                bbox_offsets = tf.boolean_mask(bbox_offsets, predictions_filter)
        ...
        if not is_training or self._debug:
            proposals_creator = SSDProposal(
                self._num_classes, self._config.proposals, self._config.variances
            )
    ...
    def loss(self, prediction_dict, return_all=False):
        ...
        with tf.name_scope("losses"):
            ...
            cls_target_one_hot = tf.one_hot(
                cls_target, depth=self._num_classes + 1, name="cls_target_one_hot"
            )
            ...
            bbox_offsets_target_positives = tf.boolean_mask(
                bbox_offsets_targets, not_ignored, name="bbox_offsets_target_positives"
            )
            ...
            safety_condition = tf.not_equal(tf.shape(bbox_offsets_positives)[0], 0)
            final_loss = tf.cond(
                safety_condition,
                true_fn=lambda: (
            ...
            prediction_dict["cls_loss_per_proposal"] = classification_loss_per_proposal

            tf.summary.scalar(
                "cls_loss", cls_loss, collections=self._losses_collections
            )

            tf.summary.scalar(
                "bbox_loss", bbox_loss, collections=self._losses_collections
            )
    ...
    def get_trainable_vars(self):
        ...
        if self._config.base_network.trainable:
            pretrained_trainable_vars = self.feature_extractor.get_trainable_vars()
            tf.logging.info(
                "Training {} vars from pretrained module.".format(
