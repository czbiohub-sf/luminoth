...
class SSDFeatureExtractor(BaseNetwork):
    def __init__(
        self, config, parent_name=None, name="ssd_feature_extractor", **kwargs
    ):
        super(SSDFeatureExtractor, self).__init__(config, name=name, **kwargs)
        if self._architecture not in VALID_SSD_ARCHITECTURES:
            raise ValueError('Invalid architecture "{}"'.format(self._architecture))
    ...
    def _build(self, inputs, is_training=True):
        ...
        if self.truncated_vgg_16_type:
            ...
            with tf.variable_scope("conv_4_3_norm"):
                ...
                vgg_conv4_3_norm = tf.nn.l2_normalize(vgg_conv4_3, 3, epsilon=1e-12)
                # Scale.
                scale_initializer = (
                    tf.ones([1, 1, 1, vgg_conv4_3.shape[3]]) * 20.0
                )  # They initialize to 20.0 in paper
            ...
            tf.summary.histogram("conv5_3_hist", vgg_network_truncation_endpoint)
