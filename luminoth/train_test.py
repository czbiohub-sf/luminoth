...
from luminoth.utils.config import get_model_config, load_config_files, get_base_config
...
class TrainTest(tf.test.TestCase):
    ...
    def get_config(self, model_type, override_params=None):
        ...
        config = get_model_config(model_base_config, custom_config, override_params)
    ...
    def testTrain(self):
        ...
        run(config, get_dataset_fn=self.get_dataset, get_model_fn=self.get_model)
    ...
    def testTrainSave(self):
        ...
        new_saver.restore(new_session, tmp_job_dir + "/test_runname/model.ckpt-3")

        # Get tensor from graph and run it in session
        w_tensor = tf.get_default_graph().get_tensor_by_name("mockfasterrcnn/w:0")
