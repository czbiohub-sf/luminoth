...
class ObjectDetectionDatasetTest(tf.test.TestCase):
    ...
    def _run_augment(self, augment_config, image, bboxes):
        ...
        with self.test_session() as sess:
            image_aug, bboxes_aug, applied_data_augmentation = sess.run(
                [image_aug, bboxes_aug, applied_data_augmentation],
                feed_dict={
                    bboxes_tf: bboxes,
                    image_tf: image,
                },
    ...
    def testRotatedAugmentation(self):
        """
        Tests that the rotate augmentation is applied in order
        """
        image = np.random.randint(low=0, high=255, size=(600, 800, 3))
        bboxes = np.array(
            [
        ...
        config = [{"rotate": {"prob": 1}}, {"rotate": {"prob": 0}}]
    ...
    def testRandomPathGaussian(self):
        ...
        config = [{"gaussian": {"prob": 1}}, {"gaussian": {"prob": 0}}]
