...
@click.option(
    "--watch/--no-watch",
    default=True,
    help="Keep watching checkpoint directory for new files.",
)  # noqa
@click.option(
    "--from-global-step",
    type=int,
    default=None,
    help="Consider only checkpoints after this global step",
)  # noqa
@click.option(
    "override_params",
    "--override",
...
@click.option(
    ...
    type=int,
    default=10,
    help="How many files per class display in every epoch.",
)  # noqa
@click.option(
    "--max-detections", type=int, default=100, help="Max detections to consider."
)  # noqa
def eval(
    dataset_split,
    config_files,
    ...
    while True:
        ...
        try:
            ...
            tf.logging.warning("Missing checkpoint; Checking again in a moment")
            time.sleep(5)
            continue
        ...
        for checkpoint in checkpoints:
            ...
            try:
                ...
                tf.logging.info("Evaluated in {:.2f}s".format(time.time() - start))
                ...
                tf.logging.info(
                    ...
                    "Checking again in a moment.".format(checkpoint["file"])
                )
                time.sleep(5)
                continue
...
def get_checkpoints(run_dir, from_global_step=None, last_only=False):
    ...
    if last_only:
        ...
        checkpoints = [c for c in checkpoints if c["global_step"] > from_global_step]
        ...
        tf.logging.info("Found {} checkpoints in run_dir".format(len(checkpoints)))
...
def evaluate_once(
    ...
    with tf.Session() as sess:
        ...
        try:
                ...
                if image_vis is not None:
                    ...
                    for gt_class in batch_gt_classes:
                        ...
                        if len(cls_files) < files_per_class:
                            files_to_visualize.setdefault(gt_class, set()).add(filename)
                            visualize_file = True
                            break
                    ...
                    if visualize_file:
                        ...
                        for image_summary in image_summaries:
                            writer.add_summary(image_summary, checkpoint["global_step"])
            ...
            tf.logging.info(
                "Finished evaluation at step {}.".format(checkpoint["global_step"])
            )
            ...
            tf.logging.info(
                "Average Precision (AP) @ [0.50:0.95] = {:.3f}".format(map_at_range)
            )
            tf.logging.info(
                "Average Recall (AR) @ [0.50:0.95] = {:.3f}".format(mar_at_range)
            )

            for idx, val in enumerate(ap_per_class[:, 0]):
                class_label = (
                    "{} ({})".format(class_labels[idx], idx) if class_labels else idx
                )
                tf.logging.debug(
            ...
            summary = [
                tf.Summary.Value(
                    tag="{}/AP@0.50".format(metrics_scope), simple_value=map_at_50
                ),
                tf.Summary.Value(
                    tag="{}/AP@0.75".format(metrics_scope), simple_value=map_at_75
                ),
                tf.Summary.Value(
            ...
            for loss_name, loss_value in val_losses.items():
                tf.logging.debug("{} loss = {:.4f}".format(loss_name, loss_value))
                summary.append(tf.Summary.Value(tag=loss_name, simple_value=loss_value))

            writer.add_summary(tf.Summary(value=summary), checkpoint["global_step"])
...
def calculate_metrics(output_per_batch, num_classes):
    ...
    """
    iou_thresholds = np.linspace(0.50, 0.95, np.round((0.95 - 0.50) / 0.05) + 1)
    # 101 recall levels, same as COCO evaluation.
    rec_thresholds = np.linspace(0.00, 1.00, np.round((1.00 - 0.00) / 0.01) + 1)
    ...
    for cls in range(num_classes):
        ...
        precision = np.divide(
            sum_true_positives.astype(float), sum_true_positives + sum_false_positives
        )
